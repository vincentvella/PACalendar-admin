...
export const validateUser = (user) => {
  const errors = {};
  if (user) {
    if (checkRequired(user.email)) {
      pushRequiredError(errors, 'Email');
    } else if (minLength(3, user.email)) {
      pushMinLengthError(errors, 'User\'s Email');
    }
    if (user.permissionLevel && user.permissionLevel > 1) {
      if (checkRequiredArr(user.orgs)) {
        pushRequiredArrError(errors, 'organization');
      }
    }
  } else {
